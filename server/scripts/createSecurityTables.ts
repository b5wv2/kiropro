...
async function migrateSecurityTables() {
  console.log('[Migration] Starting security tables migration...');
  ...
  try {
    ...
    console.log('[Migration] Creating user_sessions table...');
    await client.query(`
    ...
    console.log('[Migration] Creating security_events table...');
    await client.query(`
    ...
    // 3. user_bans table
    console.log('[Migration] Creating user_bans table...');
    await client.query(`
    ...
    console.log('[Migration] Security tables and indexes successfully created!');
  } catch (err: any) {
    await client.query('ROLLBACK');
    console.error('[Migration Error] Failed to create security tables:', err.message);
    throw err;
  } finally {
    client.release();
    await pool.end();
